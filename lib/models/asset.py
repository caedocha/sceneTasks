...
class Asset(ModelBase):
	...
	def texture_maps(self, default_dir):
			...
			if default_dir == "master":
				...
				scene_path = self.latest_master(full = True)
			else:
				default_dir_path = self.source_images().local()
				scene_path = self.latest_local(full = True)
				...
				if	maps != []:
					for current_file, next_file in ListUtils.pairwise(maps):
						...
						if next_file is not None:
							next_info = self.__chop_texture_file_name(next_file)
							if (next_info['obj'] != current_info['obj']) or (next_info['shader'] != current_info['shader']):
